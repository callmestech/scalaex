...
class Empty extends TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  override def mostRetweeted: Tweet = throw new NoSuchElementException("It's an empty tweetset")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweetAcc(src: TweetSet, acc: TweetList): TweetList = {

    if(src.isEmpty) acc else {
      val biggest = src.mostRetweeted
      val newAcc = descendingByRetweetAcc(src.remove(biggest), acc)
